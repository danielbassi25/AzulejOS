      ...
      <div className="px-4 pt-5 pb-12">
        <div className="flex items-center gap-3 mb-5">
          ...
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '8.5px', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'hsl(220,18%,60%)' }}>
            {mockMemories.length} moments archived
          </p>
              ...
              <motion.div key={memory.id}
                ...
                <Link href={`/saudade/${memory.id}`} className="block">
                  <motion.div
                    ...
                    style={{ borderRadius: '4px', border: '1px solid rgba(30,60,130,0.12)', boxShadow: '2px 4px 14px rgba(20,40,100,0.09)', height: isFeatured ? 256 : 180 }}>
                    ...
                    {/* Location */}
                    <div className="absolute top-3.5 right-3.5 flex items-center gap-1 px-2 py-1"
                      style={{ fontFamily: 'Inter, sans-serif', fontSize: '8px', fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', background: 'rgba(12,25,72,0.50)', backdropFilter: 'blur(8px)', border: '1px solid rgba(180,200,255,0.18)', borderRadius: '2px', color: 'rgba(200,215,255,0.88)' }}>
                      <MapPin className="w-2.5 h-2.5" />{memory.location}
                    </div>

                    {/* Index */}
                    <div className="absolute top-3.5 left-4" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 500, fontSize: '11px', letterSpacing: '0.05em', color: 'rgba(200,215,255,0.22)' }}>
                      {String(idx + 1).padStart(2, '0')}
                    </div>

                    {/* Tags */}
                    {memory.tags && memory.tags.length > 0 && (
                      <div className="absolute bottom-14 left-5 flex gap-1.5">
                        {memory.tags.map(tag => {
                          const tc = TAG_COLORS[tag] || { bg: 'rgba(100,100,140,0.45)', color: 'rgba(220,220,240,0.88)' };
                          return (
                            <span key={tag} style={{ fontFamily: 'Inter, sans-serif', fontSize: '7px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', background: tc.bg, color: tc.color, borderRadius: '2px', padding: '3px 8px', backdropFilter: 'blur(4px)' }}>
                              {tag}
                            </span>
                          );
                        })}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 px-5 pb-5">
                      ...
                      <h3 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 600, fontSize: isFeatured ? '1.55rem' : '1.15rem', letterSpacing: '0.01em', lineHeight: 1.2, color: 'rgba(240,238,232,0.96)', textShadow: '0 2px 12px rgba(0,0,30,0.50)' }}>
                        {memory.title}
                      </h3>
                      {isFeatured && (
